...
type NormalizedBand = 'ACCEPT' | 'REFER' | 'DECLINE'

/** Normalize band to uppercase so we handle both backend formats (Accept/ACCEPT). */
function normalizeBand(band: Band): NormalizedBand {
  return band.toUpperCase() as NormalizedBand
}

const styleMap: Record<NormalizedBand, string> = {
...
const dotMap: Record<NormalizedBand, string> = {
...
export default function BandBadge({ band, score, className }: Props) {
  ...
  return (
    ...
    >
      <span className={cn('block h-1.5 w-1.5 rounded-full', dotMap[normalized])} />
      {normalized}
      {score !== undefined && (
